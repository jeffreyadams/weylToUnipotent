...
\address{The Institute of Mathematical Sciences and Department of Mathematics, The Chinese University of Hong Kong, Shatin, N.T., Hong Kong, xuhuahe@math.cuhk.edu.hk}
...
\begin{abstract}
Let $G$ be a reductive group over an algebraically closed field and $W$ be its Weyl group. In a series of papers, Lusztig introduced a map from the set $[W]$ of conjugacy classes of $W$ to the set $[G_u]$ of unipotent classes of $G$. This map, when restricted to the set of elliptic conjugacy classes $[W_e]$ of $W$, is injective. In this paper, we show that Lusztig's map $[W_e] \to [G_u]$ is order-reversing, with respect to the natural partial orders on $[W_e]$ arising from combinatorics and the natural partial order on $[G_u]$ arising from geometry. 
\end{abstract}
...
from $\Wec$ to $\Phi(\Wec) \subset \Gu$. Michel also \cite{DHM} verified the exceptional groups by computer. 
...
Our first main result is  that $\Phi$ is order reversing in a following sense.
...
Note that the unipotent classes in the classical groups are associated to certain partitions. However, the partitions associated to elliptic conjugacy classes and the unipotent classes of the same classical group, are usually the partition of different integers. For example, in type $B_n$, the elliptic conjugacy classes correspond to partitions of $n$ while the unipotent conjugacy classes correspond to certain partitions of $2n+1$. The map from partitions of $n$ to the partitions of $2n+1$ induced from Lusztig's map in characteristic $0$ (and any characteristic $\neq 2$) are rather complicated; however, in characteristic $2$, is rather simple and is essentially the map $\alpha \mapsto (2 \alpha, 1)$. We then use the Lusztig-Spaltenstein map from the set of unipotent classes in characteristic $0$ to the set of unipotent classes in characteristic $2$ to deduce the statement  \[\tag{*}\Phi(\CC_\alpha)\leu\Phi(\CC_\beta)\Leftrightarrow \alpha\le\beta\] in characteristic $0$ (and any characteristic $\neq 2$) to the statement (*) in characteristic $2$, which is obvious since the map $\alpha \mapsto (2 \alpha, 1)$. For other classical groups, the statement $\Phi(\CC_\alpha)\leu\Phi(\CC_\beta)\Leftrightarrow \alpha\le\beta$ is verified in the same way. This is done in section \ref{3-cl}.
...
Spaltenstein's map $\theta_2$. \remind{maybe modify English}
...
in terms of $\psi_\alpha$. \remind{the last sentence is not easy to understand}
\end{lemma}

\begin{proof}
This is immediate in the case of $\Sp(2n)$: $\theta_2(\CU_{\alpha,\epsilonmax^\alpha})=\CU_\alpha$, and $\psi_\alpha=0$. \remind{Is it true that $\psi=0$ in this case? Is it how $\psi$ is defined?}
...
We labels the unipotent classes in characteristic $0$ as in \cite{Spa}*{Section IV.2}.
...
$$
The second argument \remind{?} is a unipotent class in $G_2(\C)$, labeled as in \cite{Spa}, the image of
...
\newpage \remind{why new page, also why $G_2$ again?}
$G_2$:
...
\remind{the style of references [] to the left}

...
\usepackage{amsthm,amsfonts,amssymb,amsmath,amsxtra,amsrefs,array,dynkin-diagrams,pdfpages}
\usepackage{xr-hyper}
...
\remind{for odd $n$? --- Sure: $\det:O(n)/SO(n)\rightarrow\pm1$ is an isomorphism }.
...
$$
where $\theta_p\circ\pi_2=\text{id}$.

Lusztig showed in \cite[Theorem 0.4 and \S3.9]{L2} that the following diagram is commutative:
\[
\xymatrix{& [W] \ar[ld]_{\Phi_0} \ar[rd]^{\Phi_2} & \\ [G_{0, u}] \ar@{^{(}->}[rr]^{\pi_2} & & [G_{2, u}].}
\]


In the case of a classical group
$\pi_p$ is a bijection for $\p\neq 2$, and $\pi_2,\theta_2$ are defined explicitly
...
In \cite{L1}*{\S4.1} Lusztig defines a map
$[G_{0,u}]\rightarrow [G_{p,u}]$, and states without proof that it
coincides with the map of \cite{Spa}. Implicit in this is the
following result, which makes precise for classical groups the
...
Returning to Proposition \ref{p:p=2}, this gives a reduction of Theorem \ref{main} in the classical case
...
map for $G$ in characteristic $p$.
	...
	where $\alpha'=2 \a$ or $(2\alpha,1)$. By \S \ref{s:closure}, $\CU_{\alpha', \epsilonmax^{\alpha'}} \le \CU_{\b', \epsilonmax^{\b'}}$ if and only if $\a' \le \b'$, which is equivalent to $\a \le \b$.
        ...
        by $\alpha\mapsto \alpha$ or $(2\alpha,\epsilonmax^\alpha)$ respectively, and the same argument applies.
\end{proof}
...
We prove the Proposition in the next section.
...
Let $\d$ be the permutation of $\{\pm1, \dots, \pm n\}$ such that
$\d(1) = -1$ and $\d(k) = k$ for $2 \le k \le n$. The map
$w \mapsto \d(w) := \d w \d\i$ is an outer involution of $W$. The map
$W \to W\d: w \mapsto w\d$ induces a bijection from the set of
$\d$-twisted conjugacy class of $W$ to the set of ordinary
$W$-conjugacy classes of $W\d$.
...
\end{proposition}
  ...
  $\alpha \ge \beta$, then, just as in the proof in types B/C, 
  ${}^2\CC_{\alpha} \preceq {}^2\CC_{\beta}$ follows
  from the explicit constructions of
  minimal length elements $w_{\alpha}$ and $w_{\beta}$ in
  ${}^2\CC_{\alpha}$ and ${}^2\CC_{\alpha}$ respectively.
	
On the other hand, if ${}^2 \CC_{\alpha}\d \leW {}^2 \CC_{\beta}$, then there exists $w \in {}^2 \CC_{\alpha}$ with $w \leq w_{\beta}$. Let $m=\beta_1+\cdots+\beta_k$. By direct computation, we have $| w_{\beta} \d[n-m, n-m+1]|=k$. By Proposition \ref{p:bruhat}(3) $| w\d[n-m, n-m+1]| \le k$. By Proposition \ref{D-ineq}, $k \ge \min\{k'; \alpha_1+\cdots+\alpha_{k'} \ge m\}$. In other words, $\alpha_1+\cdots+\alpha_k \ge m=\beta_1+\cdots+\beta_k$. Therefore $\alpha \ge \beta$.
\end{proof}

\subsection{Case 3: Type ${}^2 A_{n-1}$} Let $W=S_n$ be the group of permutations of $\{1, 2, \cdots, n\}$. For $1 \le i, j \le n$, we define $$w[i, j]=\{-n \le k \le n; k \le i, w(k) \ge j\}.$$
...
%\begin{proof}
	...
	On the other hand, if $\CC_{\alpha} \leW \CC_{\beta}$, then
        ...
        w_{\beta}$. Hence $w \d \ge w_{\beta} \d$. Let
        $m=\beta_1+\cdots+\beta_k$. By direct computation, we
        have
        $$|w_{\beta} \d[\lceil \frac{m}{2}\rceil, n-\lfloor
        \frac{m}{2} \rfloor+1]|+| w_{\beta} \d[n-\lfloor \frac{m}{2}
        \rfloor, \lceil \frac{m}{2}\rceil+1]|=n-k.$$
	
	By Proposition \ref{p:bruhat}(1), $$|w \d[\lceil \frac{m}{2}\rceil, n-\lfloor \frac{m}{2} \rfloor+1]|+| w \d[n-\lfloor \frac{m}{2} \rfloor, \lceil \frac{m}{2}\rceil+1]|\ge n-k.$$ By Proposition \ref{A-ineq}, $k \ge \min\{k'; \alpha_1+\cdots+\alpha_{k'} \ge m\}$. In other words, $\alpha_1+\cdots+\alpha_k \ge m=\beta_1+\cdots+\beta_k$. Therefore $\alpha \ge \beta$.
\end{proof}
...
\centerline{\includegraphics[scale=.06]{G2_elliptic.eps}}
...
\centerline{\includegraphics[scale=.15]{F4_elliptic.eps}}

$E_6$:

\centerline{\includegraphics[scale=.1]{E6_elliptic.eps}}

\newpage

$E_7$:

\centerline{\includegraphics[scale=.3]{E7_elliptic.eps}}

\newpage

$E_8$:

\centerline{\includegraphics[scale=.3]{E8_elliptic.eps}}

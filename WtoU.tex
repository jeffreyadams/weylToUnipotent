...
\def\G0u{[G_{0,u}]}  %unipotent classes in G
...
Before we discuss the strategy towards Theorem \ref{main}, we make a
...
\ref{main}. For exceptional groups, we verify the statement by
...
We will apply $\Psi$ in the following setting. Consider the sequence of maps
$$
\CP(n)\overset{2\times}\rightarrow\CP_-(2n)\overset{\Gamma}\rightarrow \CP_+(2n+1)
$$
The first map takes $(\alpha_1,\dots,\alpha_\ell)$ to $(2\alpha_1,\dots,2\alpha_\ell)$.
The composition of maps is injective, and an isomorphism of ordered sets from $\CP(n)$ to its image.
...
Consider the sets $\Gu$ (respectively $\G0u$) of unipotent conjugacy classes of $G$ and $G_0$, respectively.

\begin{proposition}{\cite{Spa}*{Theorem III.5.2}}
\label{p:p=2}
  There is an injective map $\pi:\G0u\rightarrow \Gu$, such that $\pi$ is a isomorphism of partially ordered
  sets from $\G0u$ to its image.
\end{proposition}

$$
\xymatrix{
[W_e]\ar@{=}[d]\ar@{->}[r]_\Phi&[G_u]\ar@/_/[l]_\Psi\ar@{->>}[d]_\rho\\
[W_e]\ar@{->}[r]_{\Phi_0}&[G_{0,u}]\ar@/_/[l]_{\Psi_0}\ar@{^{(}->}@/_/[u]_\pi
}
$$
...
\begin{itemize}
\item $\rho$ is surjective and $\pi$ is injective
\item $\Phi_0=\rho\Phi$
\item $\Psi_0=\Psi\pi$
\item $\Phi\Psi=\text{id}$
\item $\Phi_0\Psi_0=\text{id}$
\item $\Phi_0=\Phi_0\Psi\Phi$
  \item $\Psi_0=\Psi\Phi\Psi_0$
\item $\CC\le\CC'\Rightarrow \pi(\CC)\le\pi(\CC')$
  \item $\text{codim}(\pi({\CC)})=\text{codim}(\CC)$
\end{itemize}

We want to know, for all $\CC,\CC'\in[W_e]$:

$$
\CC\le\CC'\Leftrightarrow\Phi_0(\CC)\le \Phi_0(\CC')\text{ implies }
\CC\le\CC'\Leftrightarrow\Phi(\CC)\le \Phi_0(\CC')
$$

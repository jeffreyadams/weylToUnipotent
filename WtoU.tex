...
\item The restriction to elliptic conjugacy classes $\Phi_e: [W^D_e] \to [D_u]$ is injective. 
\end{itemize}
...
Now we recall the partial order on $[W^D_e]$ introduced in \cite[\S 4.7]{He07}. Let $\CC, \CC' \in [W^D_e]$. We denote by $\CC_{\min}$ (respectively $\CC'_{\min}$) the set of minimal length elements in $\CC$ (respectively $\CC'$). Then the following conditions are equivalent:
...
If these conditions are satisfied, then we write $\CC' \leW \CC$. By the equivalence of the conditions (1) and (2) above, the relation $\leW$ is transitive. This gives a natural partial order on the set $[W^D_e]$.
...
The condition (3) will be used to study the partial order on $[W^D_e]$ for exceptional groups. 

Now we state the main theorem of the paper. 

\begin{theorem}\label{main}
The map $\Phi_e: [W^D_e] \to [D_u]^{op}$ gives a bijection from the poset $[W^D_e]$ to its image. Here $[D_u]^{op}$ is the same as $[D_u]$ as sets, but with reversed partial order. 

In other words, let $\CC, \CC' \in [W^D_e]$. Then $\CC' \leW \CC$ if and only if $\Phi(\CC) \leu \Phi(\CC')$.
\end{theorem}
...
\begin{proof}
  ...
  \end{cases}
  $$
\remind{I think that the formula above is a bit confusing}  
...
\subsection{Parametrization of elliptic conjugacy classes} For classical groups the elliptic conjugacy classes in $W$ are parametrized as follows.  Recall $\CP(n)$ is the set of partitions of
$n$, and $\CP(n)_0$ is the subset of $\CP(n)$ consisting of partitions with an even number of parts. Let $\CP^{odd}(n)$ be the set of partitions of $n$ consisting of only odd parts. 
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item $[W^D_e]$ is parametrized by $\CP^{odd}(n)$. 
	\end{itemize}
	\item $G=SO(2n+1)$ or $Sp(2n)$: $[W_e]$ is parametrized by $\CP(n)$. 
	\item $G=O(2n)$. Then $G$ has two connected components: $G^0=SO(2n)$ and $D=G\backslash G^0$. We have that $[W_e]$ is parametrized by $\CP(n)$. Moreover, 
	\begin{itemize}
		\item $[W^{G^0}_e]$ is parametrized by $\CP(n)_0$; 
		\item $[W^D_e]$ is parametrized by $\CP(n) \backslash \CP(n)_0$. 
	\end{itemize}
\end{enumerate}
...
Suppose $G$ is a classical group. Lusztig's map $\Phi_e: [W^D_e] \to [D_u]$ is described explicitly in
...
If $p=2$, then $D$ contains unipotent elements. In this case, the map $\Phi_e: [W^D_e] \to [D_u]$ is given by $\CC_\alpha\mapsto \CU_{\alpha,\epsilonmax^\alpha}$ for $\a \in \CP^{odd}(n)$. 
...
\item[(ii)]  $p=2$: the map $\Phi_e$ is given by $\CO_{\alpha} \mapsto \CU_{(2 \alpha,2 \epsilonmax^\alpha)}$.
\end{itemize}
...
Note that the unipotent classes $\CU_{2\alpha,\epsilonmax^{2\alpha}}$ are single $G^0$-conjugacy classes.
\end{enumerate}
...
algebraically closed field $\BF$ of characteristic $p>0$. Let $G_0$
be the complex group with the same root datum as $G$.  Consider the
sets $[G_{p, u}]$ (respectively $[G_{0, u}]$) of unipotent conjugacy classes of $G_p$
and $G_0$, respectively.
...
Thus to prove Theorem \ref{main} for $Sp(2n)$ or $SO(n)$, it suffices to consider these groups over characteristic $2$. Note that the explicit description of $\Phi_2 \mid_{[W_e]}$ is rather simple. We have that

\begin{proposition}
Let $G$ be a classical group defined over an algebraically closed field of characteristic $p \ge 0$. Let $\CO_\a, \CO_\b$ be elliptic conjugacy classes of $W$. Then $\Phi(\CO_\a) \leu \Phi(\CO_\b)$ if and only if $\a \le \b$.
\end{proposition}

\begin{proof}
	If $p \neq 2$, then $[G_{p, u}]$ and $[G_{0, u}]$ are the same as posets. Moreover, the map $\pi_2: [G_{0, u}] \to [G_{2,u}]$ is an isomorphism of the posets from $[G_{0, u}]$ to its image. In other words, $\Phi_p(\CO_\a) \leu \Phi_p(\CO_\b)$ if and only if $\Phi_2(\CO_\a) \leu \Phi_2(\CO_\b)$. Here $\Phi_p$ is the Lusztig's map for $G$ in characteristic $p$. 
	
	Now by \S \ref{explicit}, for $G=Sp(2n)$ or $O(n)$, the map $\Phi_2$ is given by $\CO_{\alpha} \mapsto \CU_{\alpha', \epsilonmax^{\alpha'}}$
	...
	Similar argument holds for $G=\GLdagger(n)$. 
\end{proof}
...
\section{Partial order $\leW$ for classical groups}

\subsection{Bruhat order for classical groups}

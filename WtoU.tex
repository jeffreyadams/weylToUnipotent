...
always choose $1$ whenever possible.
...
If $G=Sp(2n)$ then $\pi:\alpha\mapsto(\alpha,\epsilon)$ where $\epsilon(i)=1$ for all $i$ with $m_\alpha(i)>0$.
...
\begin{remark}
  Assuming the conjecture Lusztig's map from $[G(\C)_u]$ to $[G(\Fbartwo)_u]$ agrees with
...
the definition of $\epsilonmax^\alpha$, $\epsilonmax^\alpha(i)=1$ if and only if
there is a row of length $i$, i.e. $\alpha_j=i$ for some $j$. Given this we see
\begin{subequations}
...
$$
taking $i\mapsto j=?$ and $j\mapsto ?$,
under which decreasing $\alpha_i$ by $1$ corresponding to decreasing $\alpha_j^*$ by $1$.
...
\centerline{\includegraphics[scale=.06]{G2_elliptic.pdf}}

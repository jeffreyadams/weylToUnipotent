...
{\bf THE REST OF THIS SECTION IS WRONG}
...
{\bf As noted above the preceding Theorem is NOT correct}
...
We give an interpretation of the map function $\psi_\alpha$ in Proposition \ref{p:psi}.
...
{\bf The next corollary is not correct. Or maybe its true but the Theorem is false in characteristic 2}
\begin{corollary}
...
\item     If $W$ is of type $B_n/C_n$  then $x\le y\Rightarrow x[i,j]\le y[i,j]$ for all $-n\le i,j\le n$
  \end{enumerate}
...
$n$, and let $\CP(n)_e$ be the subset of $\CP(n)$ consisting of
...
We prove Theorem \ref{t:main} for classical group. The main step is:

\begin{proposition}
  \label{p:order_reversing}
  Suppose $W$ is of type $B_n/C_n$, respectively $D_n$, and 
and $\alpha,\beta\in\CP(n)$ (respectively  $\CP(n)_e$).
  Then
\begin{equation}
...
\begin{tabular}{llll}
  $[W]$ & $O(4,\BC)$ & $\O(4,\overline\BF_2)$& $\SO/\O$\\
    \hline\hline
  $[2]$ & $[3,1]$& $([4], *)$ & $\O$\\
...
There is one elliptic class in $W$, $4$ total. Also  $\SO(4,\BC)$  and $\SO(4,\Ftwo)$ each have $4$ unipotent orbits.

\begin{tabular}{lll}
  $[W]$ & $SO(4,\BC)$ & $\SO(4,\overline\BF_2)$\\
  \hline\hline
...
Both $\SO(8,\BC)$ and $\SO(8,\Ftwo)$ have $12$ unipotent classes.
...
\begin{tabular}{llll}
  $\#$ & $[W]$ & $SO(12,\BC)$ & $\SO(12,\overline\BF_2)$\\
  1& $[5,1]$ & $[11,1]$ & $([10,2],*)$\\
  2& $[4,2]$ & $[9,3]$ & $([8,4],*)$\\
  3& $[3,3]$ & $[7,5]$ & $([6,6],\epsilon(6)=1))$\\
  4& $[2,2,1,1]$ & $[5,5,1,1]$ & $([5,5,1,1],*)$\\
    5& $[1,1,1,1,1,1]$ & $[3,2,2,2,2,1]$ & $([2,2,2,2,2,2],\epsilon(2)=1)$\\
\end{tabular}

\medskip

Note that in $\#4$ the map on partitions from $\Wec$ to $\Gu$ in characteristic $2$ is {\it not} multiplication by $2$.

...
Our first main result is the $\Phi$ respects the partial orders in a strong sense.

\begin{theorem}\label{main}

Let $\CC, \CC' \in \Wec$. Then $\CC \preceq\CC'$ if and only if
$\Phi(\CC') \subset \overline{\Phi(\CC)}$. In particular $\Phi$ gives
an order-reversing bijection from $\Wec$ to
its image in $\Gu$.
\end{theorem}

This holds also in the twisted setting.

Next we give a conceptual definition of Lusztig's map restricted to
$\Wec$ for classical groups. For this introduction we consider
...
both $\Wc$ and $\Gu$ are paraemtrized by partitions of $m$, and $\Phi^*$ corresponds to the identity map on partitions.
The bottom row is Lusztig's map for $G$.
Suppose $\CC$ is a unipotent class in $G^*$.
Then $\Gamma(\CC)$ is the unique open orbit in $\overline{\CC}\cap G$;
according to our next main result $\Gamma$ is well-defined, and makes the diagram commute.

\begin{theorem}
\label{second}
Suppose $G=GL(n), SO(n),O(n)$ or $Sp(2n)$.   If  $\CC\in\Wec$ then
$\overline{\Phi^*(\iota(\CC))}\cap G$ has a  unique maximal orbit for $G_u$, and
$$
\Phi(\CC)=\Gamma(\Phi^*(\iota(|CC))).
$$
\end{theorem}

\remind{Of course I'm not sure this is true, it requires careful checking.}

A twisted version of this holds as well.

Write $\CP(n)$ for the partitions of $n$.
We  identify these with Young diagrams equipped with their natural
partial order as usual (see Section ?)
If $\alpha=(\alpha_1,\alpha_2,\dots)$ is a partition of $n$ write
$2\alpha$ for the partition $(2\alpha_1,2\alpha_2,\dots)$ of $2n$.

\begin{example}
Let $G=Sp(2n)$ and $p\ne 2$. Then $\Wec$ is parametrized by $\CP(n)$,
  and $\Gu$ is parametrized by partitions of $2n$ for which
  each odd row has even multiplicity.
  Suppose
  $\CC\in \Wec$ corresponds to a partition $\alpha\in\CP(n)$. Then
  $\iota(\CC)$ corresponds to $2\alpha\in\CP(2n)$,
which is the partition for the   corresponding unipotent class $\CO^*$ in $GL(2n)$.
Then $\CO^*\cap Sp(2n)$ is a single $Sp(2n)$-class, also given by $2\alpha$. This gives the bottom row $\Phi$: restricted
  to elliptic conjugacy classes  is given on partitions by $\alpha\rightarrow 2\alpha$. 

  Now consider the case $G=O(2n)$, $p\ne 2$. In this case $\Wec$ is of
  type $C_n$ as before (since we're taking the disconnected orthogonal
  group), and $\Gu$ is parametrized by partitions of $2n$ in which
  every {\it even} row has even multiplicity.
  The maps $\iota$ and
  $\Phi^*$ are exactly the same as for $Sp(2n)$. However the unipotent
  class $2\alpha$ does not intersect $O(2n)$, since $2\alpha$ is not a
  valid partition of a unipotent class in $O(2n)$.
  According to \cite{L1}  $\Phi(\CC)$ corresponds to a certain partition $\alpha'\le \alpha$. In fact this is the
  largest such partition which corresponds to a class for $\O(2n)$, and 
this is the largest orbit in $\overline{\CO^*}\cap O(2n)$.
...
Suppose $(\alpha,\epsilon),(\beta,\delta)$ are  both in $\CJ(n)$
or $\CI(2n)$. In the case of $O(2n)$ assume they are both in $\CI_0(2n)$ or $\CI_1(2n)$. 
...
By \cite[\S 4.2]{L1} and \cite[\S 3.7 \& \S 5.5]{L3}, the map $\Phi: \Wec \to \Gu$ for classical groups are described explicitly as follows.

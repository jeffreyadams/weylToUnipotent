...
\usepackage{amsthm,amsfonts,amssymb,amsmath,amsxtra,amsrefs,array}
\usepackage{xr-hyper}
...
\title[Partial orders on conjugacy classes]{Partial orders on conjugacy classes in the Weyl group and on unipotent conjugacy classes}
...
field and let $W$ be the Weyl group of $G$. Let $\Gu$ be the set of
...
Roughly speaking, the map $\Phi$ is constructed as follows.Let
$C \in \Wc$ and $w \in C_{\min}$ be a minimal length element of
$C$. We look at the intersection of Bruhat double coset $B w B$ with
various unipotent conjugacy classes and we select the minimal
...
\begin{theorem}
\label{t:main}
Let $\CC, \CC' \in \Wec$. Then $\CC \preceq\CC'$ if and only if
$\Phi(\CC') \subset \overline{\Phi(\CC)}$.
\end{theorem}
...
$\Gu$ is not. For classical groups the interesting case is where $\F$
has characteristic $2$.
Let $G$ be a classical group  group over a field of characteristic $2$,
and let $G_0$ be the group over $\BC$ with the same root datum as $G$.
There is a natural
injective map $\pi:[G_{0,u}]\hookrightarrow \Gu$ which is an
isomorphism of partially ordered sers from $[G_{0,u}]$ to its image.
...
For simplicity, we only discuss the split groups here but the result
holds in general.
...
are parametrized by certain partitions. We use the explicit
description of the Bruhat order for the Weyl groups of classical type
to relate the partial order $\preceq$ on $\Wec$ to the natural partial
order on the partitions. It is known that the unipotent conjugacy
classes for classical groups are parametrized by some partitions. Note
that in general, the partitions arising from Weyl group conjugacy
classes and the unipotent conjugacy classes are very different. For
example, in type $B_n$, the elliptic conjugacy classes correspond to
partitions of $n$ while the unipotent conjugacy classes correspond to
certain partitions of $2n+1$. Finally, we use geometry to prove that
the partial orders on the two sets of partitions are compatible.
...
the set of non-singular bilinear forms $\phi:V\times V\rightarrow \F$.
...
is even the unipotent conjugacy classes in $\SO(2n)$ are parametrized
...
For $n\ge 1$ define $\wtcpminus(2n)$ to be the set of pairs $(\alpha,\epsilon)$ 
where
\begin{enumerate}
...
\begin{proposition}
  ...
  in bijection with $\wtcpminus(2n)$.
\end{proposition}
...
unipotent then $\phi(g)$ is a unipotent class in $G^*$, which
corresponds to a partition  $\alpha$ of $2n$, and it is easy to see $\alpha\in\CP_{-1}(2n)$.
...
$(\alpha,\epsilon)\in \wtcpminus(2n)$.


In the case of $G=O(2n)$ we need to distinguish between those
$G$-conjugacy classes contained in $\SO(2n)$ and those which are not.

Define $\wtcpminus(2n)_0\subset \wtcpminus(2n)$ to be the pairs $(\alpha,\epsilon)$ such that $\alpha_1^*$ is even, and
set $\wtcpminus(2n))1=\wtcpminus(2n)\backslash \wtcpminus(2n)_0$.

\begin{lemma}
Suppose $(\alpha,\epsilon)\in \wtcpminus(2n)$.
Then $\CU_{\alpha,\epsilon}\subset \SO(2n)$ if and only if $(\alpha,\epsilon)\in\wtcpminus(2n)_0$.
\end{lemma}

Thus $\wtcpminus(2n)_0$ (respectively $\wtcpminus(2n)_1$) is in bijection with the unipotent  $\O(2n)$ conjugacy classes in $\SO(2n)$ (respectively 
$\O(2n)\backslash\SO(2n)$).

Finally we consider 
unipotent $\SO(2n)$-conjugacy classes. 
If $(\alpha,\epsilon)\in \wtcpminus(2n)_0$ then 
...
$\wtcpminus(2n)$ and $\CJ(n)$ defined in the previous section.
...
Suppose $(\alpha,\epsilon),(\beta,\delta)$ are  both in $\wtcpminus(n)$
or $\wtcpminus(2n)$. In the case of $O(2n)$ assume they are both in $\wtcpminus(2n)_0$ or $\wtcpminus(2n)_1$. 
...
In this section $G$ is a connected reductive group, defined over $\F$ of characteristic $p>0$.
Let $G_0$ be the complex  group with the same root datum as $G$.
Consider the sets $\Gu$ (respectively $\Gou$) of unipotent conjugacy classes of $G$ and $G_0$, respectively.

\begin{proposition}{\cite{Spa}*{Theorem III.5.2}}
\label{p:p=2}
  There is an injective map $\pi:\Gou\rightarrow \Gu$, such that $\pi$ is a isomorphism of partially ordered
...
reducing Theorem \ref{t:second} to characteristic $0$.  The sets $\Gu$
...
\begin{tabular}{lll}
  $G$ & $\BC$ & $\F_2$\\\hline
  $Sp(2n)$ & $\CP_{-1}(2n)$ & $\wtcpminus(2n)$ \\
  $O(2n+1)$ & $\CP_{1}(2n+1)$ & $\wtcpminus(2n)$\\
  $O(2n)$ & $\CP_{1}(2n)$ & $\wtcpminus(2n)$\\
\end{tabular}

\bigskip

In the case $G=Sp(2n)$ the map $\pi$ is: $\pi(\alpha)=(\alpha,\epsilon)$ where $\epsilon(i)=1$;
here and elsewhere $\epsilon(i)=1$ is shorthand for: $\epsilon(i)=1$ for all $i$ with $c_\alpha(i)>0$.
...
in the terminology of [Collingwood McGovern].
...
$$
This is a partition of of $n+\kappa_\ell$. Note that if we write $\alpha+\psi_\alpha=(\alpha_1',\dots,\alpha_\ell')$, and 
if $2i+1\le\ell$ then
$$
...
We give an interpretation of the map $\psi_\alpha$ in Proposition \ref{p:psi}.
...
and $\Phi:\CC_\alpha\mapsto \CU_\alpha$.
...
\item[(i)] $p \neq 2$: $\Phi:\CO_{\alpha} \mapsto \CU_{\alpha'}$, where
  $$
  \alpha'=\begin{cases}
2\alpha+\psi_{2\alpha},    & \text{ if $\alpha$ has an odd number of parts };
\\ (2\alpha+\psi_{2 \alpha}, 1), & \text{ if $\alpha$ has an even number of parts}.
\end{cases}
$$
\item[(ii)] $p=2$: $\Phi:\CO_{\alpha}=\CU_{\alpha',\epsilon}$
  where $\alpha'=(2\alpha,1)$ and $\epsilon(i)=1$ for all $i$ with $c_\alpha(i)\ne 0$.
\end{itemize}

\item Type $C_n$. The elliptic conjugacy classes of $W$ are parametrized by $\CP(n)$.
\begin{itemize}
\item[(i)]  $p \neq 2$: $\Phi:\CO_{\alpha} \mapsto \CU_{2 \alpha}$.
\item[(ii)]  $p=2$: $\Phi:\CO_{\alpha} \mapsto \CU_{(2 \alpha,\epsilon)}$ where $\epsilon(i)=1$ for all $i$ with $c_\alpha(i)\ne 0$.
\end{itemize}
...
\begin{itemize}
\item[(i)]$p \neq 2$: $\Phi:\CO_{\alpha} \mapsto \CU_{2 \alpha+\psi_{2 \alpha}}$.
\item[(ii)]  $p=2$: $\Phi:\CO_{\alpha}\mapsto\CU_{(2\alpha,\epsilon)}$ where $\epsilon(i)=1$ for all $i$ with $c_\alpha(i)\ne 0$.
\end{itemize}
...
Then $\Phi:\CO_{\alpha}\mapsto \CU_{2\alpha,\epsilon}$ where $\epsilon(i)=1$ for all $i$ with $c_\alpha(i)\ne 0$.
\end{itemize}


The function $\psi$ has an interpretation in terms of the map $\rho$ of the previous section.

\begin{proposition}
\label{p:psi}
Let $G=Sp(2n),O(2n+1)$ or $O(2n)$ over a field of characteristic $2$.
Suppose $\CC\in\Wec$ and $\CU=\Phi(\CC)\in \Gu$.

Write $\CU=\CU_{\alpha,\epsilon}$ for $(\alpha,\epsilon)\in\wtcpminus(2n)$.
...
$$
\xymatrix{
[W_e]\ar@{=}[d]\ar@{->}@/_/[r]_\Phi&\Gue\ar@/_/[l]_\Psi\ar@{->}@/_/[d]_\rho\\
[W_e]\ar@{->}@/_/[r]_{\Phi_0}&\Goue\ar@/_/[l]_{\Psi_0}\ar@{->}@/_/[u]_\pi
}
$$
All of the arrows in the diagram are bijections.  The top row is
Lusztig's map $\Phi$ for $\Gu$, restricted to the elliptic elements,
on which it has an inverse $\Psi$. The bottom row is the same construction
for $G_0$.  The maps $\pi,\rho$ are those of Section \ref{s:p=2},
and these are isomorphisms of partially ordered sets.


\begin{corollary}
\label{c:reduction} It is enough to prove Theorem \ref{t:second} in characteristic $0$.
\end{corollary}

...
In this section, we recollect some facts on the unipotent conjugacy classes of classical groups.

Let $\CP(n)$ be the set of partitions of $n$. For $\k=\pm 1$, let $$\CP_\k(n)=\{\a=(a_1, \ldots, a_l) \in \CP(n); \sharp\{j; a_j=i\} \text{ is even for all $i$ with } (-1)^i=\k\}.$$

\subsection{Good prime case} We first recall the case where the characteristic is $0$ or a good prime.

\begin{itemize}
	\item Type $A_{n-1}$: Unipotent conjugacy classes are parametrized by the set $\CP(n)$.
	
	\item Type $B_n$: Unipotent conjugacy classes are parametrized by the set $\CP_1(2n+1)$.
	
	\item Type $C_n$: Unipotent conjugacy classes are parametrized by the set $\CP_{-1}(2n)$.
	
	\item Type $D_n$: Unipotent conjugacy classes are parametrized by the set of partitions in $\CP_1(2n)$ with even number parts, except that ``very even'' partition (i.e, the partitions with only even parts, each having even multiplicity) corresponds to two orbits.
\end{itemize}

Moreover, let $G$ be a classical group and let $\a, \b$ be the partitions of two unipotent conjugacy classes $\CU_{\a}$ and $\CU_{\b}$ of $G$. Then $\CU_{\a} \subset \overline{\CU_{\b}}$ if and only if $\a \le \b$.

\subsection{Bad prime case} Now we discuss the classical groups of type $B_n$, $C_n$ or $D_n$ in characterstic $2$. The unipotent classes in these groups were determined by G. E. Wall \cite{Wall}.
...
\subsection{Lusztig's map} We recall the function $\psi$ in \cite[\S 1.6]{L1}
